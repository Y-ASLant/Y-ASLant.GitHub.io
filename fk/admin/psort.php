...
if($action=="add"){
  ...
  if($S_title!=""){
    sql("insert into sl_psort(S_title,S_order,S_mid) values('$S_title',$S_order,0)");
...
$S_counts=getrs("select count(S_id) as S_count from sl_psort where S_del=0 and S_mid=0","S_count");
?>
<!DOCTYPE html>
<html lang="zh">
<head>
        ...
        <div class="row">
                <div class="col-lg-5">
                  ...
                  <div class="card card-primary">
                        <ul class="list-group">
                          <li class="list-group-item" style="background: #f7f7f7"><div class="part">自营分类</div><div class="part2">上架</div></li>
                          <?php 
                            $list=getlist("select * from sl_psort where S_mid=0 and S_del=0  order by S_order asc,S_id desc");
                            foreach($list as $S){
                              if($S["S_id"]==$S_id){
                                    $active="active";
                                  }else{
                  ...
                  <div class="card card-primary">
                        ...
                        <ul class="list-group">
                          <li class="list-group-item" style="background: #f7f7f7"><div class="part">入驻商家分类</div><div class="part2">上架</div></li>
                          <?php 
                            $list=getlist("select * from sl_psort where S_mid>0 and S_del=0  order by S_order asc,S_id desc");
                            foreach($list as $S){
                              if($S["S_id"]==$S_id){
