...
$S_count=intval(getrs("select count(S_id) as S_count from sl_psort where S_mid=".$M_id,"S_count"));

if($P_id!=""){
  $aa="edit&P_id=".$P_id;
  $P=getrs("select * from sl_product where P_mid=$M_id and P_id=".$P_id);
  $P_title=$P["P_title"];
  $P_pic=$P["P_pic"];
...
if($P_price>0){
  if($P_title!="" && $P_content!=""){
    sql("insert into sl_product(P_title,P_pic,P_content,P_price,P_sort,P_time,P_sell,P_selltype,P_card,P_use,P_mid,P_sh) values('$P_title','$P_pic','$P_content',$P_price,$P_sort,'$P_time','$P_sell',$P_selltype,'$P_card','$P_use',$M_id,$P_sh)");
    $P_id=getrs("select * from sl_product where P_title='$P_title'  and P_sort=$P_sort","P_id");
    if($C_sh==1){
        sendmail("商品审核通知","<p>您的网站《".$C_webtitle."》有用户发布商品，请尽快审核</p><p>商品名称：$P_title</p><p>商品价格：$P_price</p><p>发布时间：$P_time</p>",$C_recieve);
    }
                  ...
                  <div class="card card-primary">
                    <div class="card-header">
                      <h4>商品列表</h4>
                        ...
                        <ul class="list-group">
                        <?php 
                        $list=getlist("select * from sl_product where P_del=0 and P_mid=$M_id  order by P_id desc limit 10");
                        foreach($list as $row){
                          if($row["P_id"]==$P_id){
                            $active="active";
                          }else{
                    ...
                    foreach($list as $row){
                        ...
                        <div class="form-group row">
                          ...
                          <div class="col-md-10">
                            ...
                            <?php if($C_sh==1){echo "<span style=\"font-size:12px;color:#33cabb\">说明：需管理员审核后才可发布</span>";}?>
                          </div>
                        </div>
                    </div>
