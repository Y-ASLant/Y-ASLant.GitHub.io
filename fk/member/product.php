...
if($action=="del"){
    $P_id=intval($_GET["P_id"]);
    sql("update sl_product set P_del=1 where P_id=$P_id and P_mid=$M_id");
    die("success");
}
...
if($action=="on"){
    $P_id=intval($_GET["P_id"]);
    $on=intval($_GET["on"]);
    sql("update sl_product set P_on=$on where P_id=$P_id and P_mid=$M_id");
    if($on==1){
...
if($action=="delall"){
  ...
  if(count($id)>0) {
    ...
    for ($i=0 ;$i<count($id);$i++) {
      sql("update sl_product set P_del=1 where P_mid=$M_id and P_id=".intval($id[$i]));
      $shu=$shu+1 ;
      $ids=$ids.$id[$i].",";
    }
...
if($action=="save"){
	foreach ($_POST as $x=>$value) {
		if(splitx($x,"_",0)=="title"){
			sql("update sl_product set P_title='".$_POST[$x]."' where P_mid=$M_id and P_id=".intval(splitx($x,"_",1)));
		}
		if(splitx($x,"_",0)=="order"){
			sql("update sl_product set P_order=".$_POST[$x]." where P_mid=$M_id and P_id=".intval(splitx($x,"_",1)));
		}
		if(splitx($x,"_",0)=="sort"){
			sql("update sl_product set P_sort=".$_POST[$x]." where P_mid=$M_id and P_id=".intval(splitx($x,"_",1)));
		}
		if(splitx($x,"_",0)=="price"){
			sql("update sl_product set P_price=".$_POST[$x]." where P_mid=$M_id and P_id=".intval(splitx($x,"_",1)));
		}
	}
	die("success");
}

$P_all=getrs("select count(P_id) as P_count from sl_product where P_mid=$M_id and P_del=0","P_count");
?>
<!DOCTYPE html>
<html lang="zh">
<head>
                  ...
                  <div class="card card-primary">
                    ...
                    <div class="card-header">
                      <h4>商品分类列表
                        ...
                        <ul class="list-group">
                        <li class="list-group-item" style="background: #f7f7f7"><div class="part">分类名称 [商品数量]</div><div class="part2">上架</div></li>
                          <a href="?S_id=0" class="list-group-item <?php if($S_id==0){echo "active";}?>"><div class="part"><b>全部商品 [<?php echo $P_all?>个]</b></div></a>
                          <?php 
                            $list=getlist("select * from sl_psort where S_mid=$M_id and S_del=0 order by S_id desc");
                            foreach($list as $S){
                              $P_count=getrs("select count(P_id) as P_count from sl_product where P_mid=$M_id and P_sort=".$S["S_id"]." and P_del=0","P_count");
                ...
                <div class="col-lg-9">
                  ...
                  <div class="card card-primary">
                    <div class="card-header ">
                      ...
                      <a class="btn btn-warning btn-sm pull-right" href="<?php echo $url?>" target="_blank"><i class="mdi mdi-cart"></i> 店铺首页</a>
                    </div>
<form id="form">
<table class="table">
  ...
  <?php
    if($S_id==0){
        $sql="select * from sl_product where P_del=0 and P_mid=$M_id order by P_id desc limit ".(($page-1)*20).",20";
      }else{
        $sql="select * from sl_product where P_del=0 and P_mid=$M_id and P_sort=".$S_id." order by P_id desc limit ".(($page-1)*20).",20";
      }

      $list=getlist($sql);
      foreach($list as $P){
        if($P["P_sh"]==0){
            $sh="<span class=\"label label-outline-warning\">未审核</span>";
        }
        if($P["P_sh"]==1){
        ...
        $list=getlist("select * from sl_psort where S_mid=$M_id and S_del=0 order by S_order,S_id desc");
        foreach($list as $row2){
            if($row2["P_sort"]==$P["S_id"]){
                  $selected="selected";
                }else{
                  $selected="";
                }
